...
"$PYTHON" scripts/benchmark/split_grouped_prompts_for_transfer.py \
  ...
  --output-root "$OUT_ROOT/split" --dataset qasper --train-ratio 0.6 --seed 0

echo "--- [3a] train trace run (Prefetch-B active, prefix caching off) ---"
env "${COMMON_ENV[@]}" \
  bash scripts/entrypoints/run_grouped_exact_next_prefetch_ablation.sh \
    --grouped-root "$OUT_ROOT/split/train" --datasets qasper --limit "$LIMIT" \
...
env "${COMMON_ENV[@]}" \
  ...
  bash scripts/entrypoints/run_grouped_exact_next_prefetch_ablation.sh \
    --grouped-root "$OUT_ROOT/split/test" --datasets qasper --limit "$LIMIT" \
